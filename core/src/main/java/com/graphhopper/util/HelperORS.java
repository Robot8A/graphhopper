...
import com.graphhopper.storage.OSMMultipolygonExtension;
...
public class HelperORS {
    ...
    public static OSMMultipolygonExtension getOSMMultipolygonExtensions(GraphExtension extendedStorage) {
        if (extendedStorage instanceof OSMMultipolygonExtension) {
            return (OSMMultipolygonExtension) extendedStorage;
        } else if (extendedStorage instanceof ExtendedStorageSequence) {
            ExtendedStorageSequence ess = (ExtendedStorageSequence) extendedStorage;
            GraphExtension[] exts = ess.getExtensions();
            for (int i = 0; i < exts.length; i++) {
                if (exts[i] instanceof OSMMultipolygonExtension) {
                    return (OSMMultipolygonExtension) exts[i];
                }
            }
        }
